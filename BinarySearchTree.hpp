...
public:
  ...
  BinarySearchTree(std::vector<int> &nums); //special constructor that builds from a sorted vector
  ...
  /**Rearrange the tree to be in-order (as in in-order traversal)*/
  ...
  /**Return true if BST is full, false otherwise*/
  bool isFull() const;


//@param p a reference to the 1st employee for which we want to find the "lowest common "direct supervisor
//@param q a reference to the 2nd employee for which we want to find the "lowest common "direct supervisor
//@return the name (a string) of the "lowest common" direct supervisor of p and q
  std::string getDirectSupervisor(Employee &one, Employee &two) const;
...
private:
  ...
  //display helpers
  //helper functions that assist with the display of the program
  void preorderHelper(std::shared_ptr<BinaryNode<T>> node);
  
  void inOrderHelper(std::shared_ptr<BinaryNode<T>> root, std::vector<T>& values);
  
  bool isBalancedHelper(std::shared_ptr<BinaryNode<T>> root) const;
  
  bool isCompleteHelper(std::shared_ptr<BinaryNode<T>> node, int index, int count) const;
  
  bool isFullHelper(std::shared_ptr<BinaryNode<T>> node) const;
  

  //helper functions that assist with the getDirectSupervisor BST method
  std::shared_ptr<BinaryNode<T>> constructorHelper(std::vector<T>& values, int start, int end);
  
  std::shared_ptr<BinaryNode<T>> findEmployee(std::shared_ptr<BinaryNode<T>> node, Employee emp) const;
  
  std::shared_ptr<BinaryNode<T>> lca(std::shared_ptr<BinaryNode<T>> node, std::shared_ptr<BinaryNode<T>> emp1, std::shared_ptr<BinaryNode<T>> emp2) const; //least common ancestor
};
